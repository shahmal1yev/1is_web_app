...
<section class="vacancy-section" style="background-image:linear-gradient(0deg, rgba(4, 15, 15, 0.6), rgba(32, 34, 80, 0.6)),
      ...
      <form class="main-filter" method="GET" action="{{route('compsearch')}}">
        <div class="active-filters">
          <div class="filter1">
            ...
            <input class="filter-input" placeholder="@lang('front.compaxtar')" type="search" name="query" value="{{ old('query', $compname) }}"/>
          </div>
    ...
    <section class="all-vacantions">
                ...
                <div class="select_box">
                   <select class="vac-select" name="sector" onchange="this.form.submit()">
                    ...
                    @foreach($sectors as $sector)
                        <option value="{{$sector->id}}"<?php if($selected_sector == $sector->id) echo ' selected'; ?>>{{$sector->title_az}}</option>
                    @endforeach
        ...
        <div class="company-div all-comp-container">            
          ...
          <p style="margin-left: 15px; font-size: 24px;"><span style="color: #F96D00;">{{ $compname }}</span> açar sözünə uyğun nəticə tapılmadı </p>
          @else
                @foreach($allcompanies as $key=>$allcomp)

          <a href="{{route('compdetail', $allcomp->id)}}" class="col-lg-4 col-md-6">
            <div class="comp-card">
                <div class="inner2">
