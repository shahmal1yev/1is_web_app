...
@section('title','1is | Şirkətlər')
@section('content')
    <!-- start page title -->
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                <h4 class="mb-sm-0 font-size-18">Şirkətlər</h4>
                ...
                <div class="card-body">
                    <table id="datatable-buttons" class="table table-bordered dt-responsive nowrap w-100">
                        ...
                        @foreach($companies as $key=>$company)
                            <tr>
                                <td>{{$key+1}}</td>
                                <td>{{$company->getUser() ? $company->getUser()->name : ''}}</td>
                                <td>{{$company->name}}</td>
                                <td>{{$company->getSector() ? $company->getSector()->title_az : ''}}</td>
                                <td>{{$company->view}}</td>
                                <td>
                                    <p class="d-none">{{$company->status == 1 ? "Active" : "Deactive"}}</p>
                                    <input type="checkbox" id="switch{{$company->id}}" switch="none" {{$company->status == 1 ? "checked" : ""}} onchange="changeStatus({{$company->id}})" />
                                    <label for="switch{{$company->id}}" data-on-label="On" data-off-label="Off"></label>
                                </td>
                                <td>{{ \Carbon\Carbon::parse($company->created_at)->format('j F, Y') }}</td>
                                <td>
                                    <a href="{{route('companiesEdit',$company->id)}}">
                                        <button class="btn btn-outline-warning"><i class="bx bxs-edit"></i></button>
                                    </a>
                                    <button class="btn btn-outline-danger" onclick="deleteCompanies({{$company->id}})"><i class="bx bxs-trash"></i></button>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
...
@section('js')
    ...
    <script>
        ...
        const changeStatus = (id) => {
            ...
            $.ajax({
                type: "POST",
                url: "/admin/companies/change/status",
                data: {
                    _token: CSRF_TOKEN,
                    id,
                },
