...
@section('title','1is | İstifadəçi şərtləri')
@section('content')
    <!-- start page title -->
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                <h4 class="mb-sm-0 font-size-18">İstifadəçi şərtləri</h4>

                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        <li class="breadcrumb-item"><a href="{{route('adminIndex')}}">Ana səhifə</a></li>
                        <li class="breadcrumb-item active">İstifadəçi şərtləri</li>
                    </ol>
                </div>
            ...
            <div class="card">
                <div class="card-body">
                    <div class="col-12 d-flex justify-content-end mb-4">
                        <button class="btn btn-success" onclick="addPolicy()">İstifadəçi şərti əlavə et</button>
                    </div>
                    <table id="datatable-buttons" class="table table-bordered dt-responsive nowrap w-100">
                        ...
                        @foreach($policies as $key=>$policy)
                            <tr>
                                <td>{{$key+1}}</td>
                                <td>{{mb_substr($policy->content_az,0,100)}}@if(strlen($policy->content_az) > 100) ... @endif</td>
                                <td>
                                    <p class="d-none">{{$policy->status == 1 ? "Active" : "Deactive"}}</p>
                                    <input type="checkbox" id="switch{{$policy->id}}" switch="none" {{$policy->status == 1 ? "checked" : ""}} onchange="changeStatus({{$policy->id}})" />
                                    <label for="switch{{$policy->id}}" data-on-label="On" data-off-label="Off"></label>
                                </td>
                                <td>
                                    <button class="btn btn-outline-warning" onclick="getPolicy({{$policy->id}})"><i class="bx bxs-edit"></i></button>
                                    <button class="btn btn-outline-danger" onclick="deletePolicy({{$policy->id}})"><i class="bx bxs-trash"></i></button>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
    ...
    <div class="modal fade" id="getPolicy" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <form action="{{route('policyEditPost')}}" method="POST" class="modal-dialog">
            @csrf
            <div class="modal-content">
                <input type="hidden" id="edit_id" name="id">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">İstifadəçi şərti redaktə<span id="get_name"></span></h5>
    ...
    <div class="modal fade" id="addPolicy" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <form action="{{route('policyAddPost')}}" method="POST" class="modal-dialog">
            @csrf
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">İstifadəçi şərti əlavə<span id="get_name"></span></h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="mb-3">
