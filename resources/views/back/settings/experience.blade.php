...
@section('title','1is | Təcrübə')
@section('content')
    <!-- start page title -->
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                <h4 class="mb-sm-0 font-size-18">Təcrübə</h4>

                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        <li class="breadcrumb-item"><a href="{{route('adminIndex')}}">Ana səhifə</a></li>
                        <li class="breadcrumb-item active">Təcrübə</li>
                    </ol>
                </div>
            ...
            <div class="card">
                <div class="card-body">
                    <div class="col-12 d-flex justify-content-end mb-4">
                        <button class="btn btn-success" onclick="addExperience()">Təcrübə əlavə et</button>
                    </div>
                    <table id="datatable-buttons" class="table table-bordered dt-responsive nowrap w-100">
                        ...
                        @foreach($experiences as $key=>$experience)
                            <tr>
                                <td>{{$key+1}}</td>
                                <td>{{$experience->title_az}}</td>
                                <td>{{$experience->slug}}</td>
                                <td>
                                    <p class="d-none">{{$experience->status == 1 ? "Active" : "Deactive"}}</p>
                                    <input type="checkbox" id="switch{{$experience->id}}" switch="none" {{$experience->status == 1 ? "checked" : ""}} onchange="changeStatus({{$experience->id}})" />
                                    <label for="switch{{$experience->id}}" data-on-label="On" data-off-label="Off"></label>
                                </td>
                                <td>
                                    <button class="btn btn-outline-warning" onclick="getExperience({{$experience->id}})"><i class="bx bxs-edit"></i></button>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
    ...
    <div class="modal fade" id="getExperience" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <form action="{{route('settingsExperiencePost')}}" method="POST" class="modal-dialog">
            @csrf
    ...
    <div class="modal fade" id="addExperience" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <form action="{{route('settingsExperienceAdd')}}" method="POST" class="modal-dialog">
            @csrf
            <div class="modal-content">
                <input type="hidden" id="edit_id" name="id">
                <div class="modal-header">
...
@section('js')
    ...
    <script>
        ...
        const getExperience = (id) => {
            ...
            $.ajax({
                type: "POST",
                url: "/admin/settings/experience/edit",
                data: {
                    _token: CSRF_TOKEN,
                    id,
                },
