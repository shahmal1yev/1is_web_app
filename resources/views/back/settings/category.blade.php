...
@section('title','1is | Kateqoriyalar')
@section('content')
    <!-- start page title -->
    <div class="row">
        <div class="col-12">
            ...
            <div class="card">
                <div class="card-body">
                    <div class="col-12 d-flex justify-content-end mb-4">
                        <button class="btn btn-success" onclick="addCategory()">Kateqoriya əlavə et</button>
                    </div>
                    <table id="datatable-buttons" class="table table-bordered dt-responsive nowrap w-100">
                        ...
                        @foreach($categories as $key=>$category)
                            <tr>
                                ...
                                <td>{!! $category->icon !!}</td>
                                <td>
                                    <p class="d-none">{{$category->status == 1 ? "Active" : "Deactive"}}</p>
                                    <input type="checkbox" id="switch{{$category->id}}" switch="none" {{$category->status == 1 ? "checked" : ""}} onchange="changeStatus({{$category->id}})" />
                                    <label for="switch{{$category->id}}" data-on-label="On" data-off-label="Off"></label>
                                </td>
                                <td>
                                  <button class="btn btn-outline-warning" onclick="getCategory({{$category->id}})"><i class="bx bxs-edit"></i></button>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
    ...
    <div class="modal fade" id="getCategory" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <form action="{{route('settingsCategoriesPost')}}" method="POST" class="modal-dialog">
            @csrf
            <div class="modal-content">
                <input type="hidden" id="edit_id" name="id">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Kateqoriya Redaktə<span id="get_name"></span></h5>
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-4">
                        <div class="col-lg-12">
                           <p>Cari icon: <span id="edit_icon"></span></p>
                        </div>
                        <div class="col-lg-12">
                            <label for="icon" class="form-label"> İcon</label>
    ...
    <div class="modal fade" id="addCategory" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <form action="{{route('settingsCategoriesAdd')}}" method="POST" class="modal-dialog">
            @csrf
            <div class="modal-content">
                <input type="hidden" id="edit_id" name="id">
                <div class="modal-header">
...
@section('js')
    ...
    <script>
        ...
        const getCategory = (id) => {
            ...
            $.ajax({
                type: "POST",
                url: "/admin/settings/categories/edit",
                data: {
                    _token: CSRF_TOKEN,
                    id,
                },
                success: function (data) {
                    if(data != 0){
                        ...
                        document.getElementById("edit_icon").innerHTML = data.icon;

                        $('#getCategory').modal('show');
                    }else{
                        Swal.fire({
                            title: "Ooops",
