...
@section('title','1is | Sektorlar')
@section('content')
    <!-- start page title -->
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                <h4 class="mb-sm-0 font-size-18">Sektorlar</h4>
            ...
            <div class="card">
                <div class="card-body">
                    <div class="col-12 d-flex justify-content-end mb-4">
                        <button class="btn btn-success" onclick="addSector()">Sektor əlavə et</button>
                    </div>
                    <table id="datatable-buttons" class="table table-bordered dt-responsive nowrap w-100">
                        ...
                        @foreach($sectors as $key=>$sector)
                            <tr>
                                <td>{{$key+1}}</td>
                                <td>{{$sector->title_az}}</td>
                                <td>{{$sector->slug}}</td>
                                <td>
                                    <p class="d-none">{{$sector->status == 1 ? "Active" : "Deactive"}}</p>
                                    <input type="checkbox" id="switch{{$sector->id}}" switch="none" {{$sector->status == 1 ? "checked" : ""}} onchange="changeStatus({{$sector->id}})" />
                                    <label for="switch{{$sector->id}}" data-on-label="On" data-off-label="Off"></label>
                                </td>
                                <td>
                                    <button class="btn btn-outline-warning" onclick="getCategory({{$sector->id}})"><i class="bx bxs-edit"></i></button>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
    ...
    <div class="modal fade" id="getSector" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <form action="{{route('settingsSectorsPost')}}" method="POST" class="modal-dialog">
            @csrf
    ...
    <!-- sample modal content -->
    <div class="modal fade" id="addSector" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <form action="{{route('settingsSectorsAdd')}}" method="POST" class="modal-dialog">
            @csrf
            <div class="modal-content">
                <input type="hidden" id="edit_id" name="id">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Sektor əlavə<span id="get_name"></span></h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="mb-3">
...
@section('js')
    ...
    <script>
        ...
        const getCategory = (id) => {
            ...
            $.ajax({
                type: "POST",
                url: "/admin/settings/sectors/edit",
                data: {
                    _token: CSRF_TOKEN,
                    id,
                },
