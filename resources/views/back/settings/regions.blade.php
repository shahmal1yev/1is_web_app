...
@section('title','1is | Rayonlar')
@section('content')
    <!-- start page title -->
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                <h4 class="mb-sm-0 font-size-18">Rayonlar</h4>

                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        <li class="breadcrumb-item"><a href="{{route('adminIndex')}}">Ana səhifə</a></li>
                        <li class="breadcrumb-item active">Rayonlar</li>
                    </ol>
                </div>
            ...
            <div class="card">
                <div class="card-body">
                    <div class="col-12 d-flex justify-content-end mb-4">
                        <button class="btn btn-success" onclick="addRegions()">Rayon əlavə et</button>
                    </div>
                    <table id="datatable-buttons" class="table table-bordered dt-responsive nowrap w-100">
                        ...
                        @foreach($regions as $key=>$region)
                            <tr>
                                <td>{{$key+1}}</td>
                                <td>{{$region->title_az}}</td>
                                <td>{{$region->slug}}</td>
                                <td>
                                    <p class="d-none">{{$region->status == 1 ? "Active" : "Deactive"}}</p>
                                    <input type="checkbox" id="switch{{$region->id}}" switch="none" {{$region->status == 1 ? "checked" : ""}} onchange="changeStatus({{$region->id}})" />
                                    <label for="switch{{$region->id}}" data-on-label="On" data-off-label="Off"></label>
                                </td>
                                <td>
                                    <button class="btn btn-outline-warning" onclick="getRegions({{$region->id}})"><i class="bx bxs-edit"></i></button>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
    ...
    <div class="modal fade" id="getRegions" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <form action="{{route('settingsRegionsPost')}}" method="POST" class="modal-dialog">
            @csrf
            <div class="modal-content">
                <input type="hidden" id="edit_id" name="id">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Təhsil Redaktə<span id="get_name"></span></h5>
    ...
    <!-- sample modal content -->
    <div class="modal fade" id="addRegions" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <form action="{{route('settingsRegionsAdd')}}" method="POST" class="modal-dialog">
            @csrf
            <div class="modal-content">
                <input type="hidden" id="edit_id" name="id">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Təhsil Əlavə<span id="get_name"></span></h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="mb-3">
...
@section('js')
    ...
    <script>
        ...
        const getRegions = (id) => {
            ...
            $.ajax({
                type: "POST",
                url: "/admin/settings/regions/edit",
                data: {
                    _token: CSRF_TOKEN,
                    id,
                },
