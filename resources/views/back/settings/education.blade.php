...
@section('content')
            ...
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                ...
                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        ...
                        <li class="breadcrumb-item active">Təhsil</li>
                    </ol>
                </div>
            ...
            <div class="card">
                <div class="card-body">
                    <div class="col-12 d-flex justify-content-end mb-4">
                        <button class="btn btn-success" onclick="addEducation()">Təhsil əlavə et</button>
                    </div>
                    <table id="datatable-buttons" class="table table-bordered dt-responsive nowrap w-100">
                        ...
                        @foreach($educations as $key=>$education)
                            <tr>
                                <td>{{$key+1}}</td>
                                <td>{{$education->title_az}}</td>
                                <td>{{$education->slug}}</td>
                                <td>
                                    <p class="d-none">{{$education->status == 1 ? "Active" : "Deactive"}}</p>
                                    <input type="checkbox" id="switch{{$education->id}}" switch="none" {{$education->status == 1 ? "checked" : ""}} onchange="changeStatus({{$education->id}})" />
                                    <label for="switch{{$education->id}}" data-on-label="On" data-off-label="Off"></label>
                                </td>
                                <td>
                                    <button class="btn btn-outline-warning" onclick="getEducaiton({{$education->id}})"><i class="bx bxs-edit"></i></button>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
    ...
    <div class="modal fade" id="getEducation" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <form action="{{route('settingsEducationPost')}}" method="POST" class="modal-dialog">
            @csrf
    ...
    <div class="modal fade" id="addEducation" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <form action="{{route('settingsEducationAdd')}}" method="POST" class="modal-dialog">
            @csrf
            <div class="modal-content">
                <input type="hidden" id="edit_id" name="id">
                <div class="modal-header">
...
@section('js')
    ...
    <script>
        ...
        const getEducaiton = (id) => {
            const CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
            $.ajax({
                type: "POST",
                url: "/admin/settings/education/edit",
                data: {
                    _token: CSRF_TOKEN,
                    id,
                },
